...
// Maps AWS region codes to the usagetype prefix used in EKS pricing.
// Format: "{prefix}-AmazonEKS-Hours:perCluster"
const REGION_TO_EKS_PREFIX: Record<string, string | undefined> = {
  'us-east-1': 'USE1',
  'us-east-2': 'USE2',
  'us-west-1': 'USW1',
  'us-west-2': 'USW2',
...
export const eksHandler: ResourceHandler = {
  ...
  buildPricingQuery(_attributes: PricingAttributes, region: string): PricingQuery {
    ...
    const prefix = REGION_TO_EKS_PREFIX[region];
    ...
    if (prefix !== undefined) {
      filters.push({ field: 'usagetype', value: `${prefix}-AmazonEKS-Hours:perCluster` });
    }

    return { serviceCode: 'AmazonEKS', filters };
  },
